...
class Options{
	...
	private static $table = 'page_options';
	...
	private static function _option($name, $value = null, $attributes = null) {
		$option = new Option();
		$option->name = $name;
	...
	public static function getForms() {
		...
		foreach (array(
			'PAGE_HEAD_META_NAME',
			'PAGE_HEAD_LINK_REL',
			'PAGE_HEAD_META_HTTP_ENQUIV',
			'PAGE_HEAD_META_NAME_OG',
			'PAGE_HEAD_META_NAME_FACEBOOK',
			'PAGE_HEAD_META_NAME_TWITTER',
			'PAGE_HEAD_META_NAME_MSAPPLICATION'
		) as $k) {
			$form = new Form($k);
			$form->caption = lang($k);
			foreach (call_user_func(array($k, 'getLabels')) as $v => $l) {
				$form->addElement($v, 'text', array(
					'label' => $v,
					'hint' => $l
				));
			}
