...
class Url {
	...
	protected function initDefaultUri($uri) {
		switch ($uri) {
			...
			case 'referer':
			case 'back':
				if (($referer = $this->getServer('HTTP_REFERER'))) {
					return $referer;
				}
				break;
		}
	...
	public function parse($uri) {
		...
		if (is_string($uri)) {
			if (null !== ($u = $this->initDefaultUri($uri))) {
				$uri = $u;
			}
		} elseif (is_array($uri)) {
			$params = $uri;
	...
	public function toString() {
		...
		$uri = $this->getHome();
		if ($this->path) {
			$uri .= trim($this->path, self::URI_DELIMITER) . self::URI_DELIMITER;
		}
		if ($this->query) {
			$uri .= "?" . $this->query;
		}
