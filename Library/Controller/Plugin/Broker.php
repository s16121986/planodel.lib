...
/** Controller_Plugin_Abstract */
require_once 'Library/Controller/Plugin/Abstract.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Controller_Plugin_Broker extends Controller_Plugin_Abstract
{

    /**
     * Array of instance of objects extending Controller_Plugin_Abstract
     *
     * @var array
     */
     ...
     * Register a plugin.
     *
     * @param  Controller_Plugin_Abstract $plugin
     * @param  int $stackIndex
     * @return Controller_Plugin_Broker
     */
    public function registerPlugin(Controller_Plugin_Abstract $plugin, $stackIndex = null)
    {
        if (false !== array_search($plugin, $this->_plugins, true)) {
            ...
            throw new Controller_Exception('Plugin already registered');
        }
        ...
        if ($stackIndex) {
            if (isset($this->_plugins[$stackIndex])) {
                require_once 'Library/Controller/Exception.php';
                throw new Controller_Exception('Plugin with stackIndex "' . $stackIndex . '" already registered');
            }
     ...
     * Unregister a plugin.
     *
     * @param string|Controller_Plugin_Abstract $plugin Plugin object or class name
     * @return Controller_Plugin_Broker
     */
    public function unregisterPlugin($plugin)
    {
        if ($plugin instanceof Controller_Plugin_Abstract) {
            // Given a plugin object, find it in the array
            $key = array_search($plugin, $this->_plugins, true);
            if (false === $key) {
                require_once 'Library/Controller/Exception.php';
                throw new Controller_Exception('Plugin never registered.');
            }
     ...
     * @return false|Controller_Plugin_Abstract|array Returns false if none found, plugin if only one found, and array of plugins if multiple plugins of same class found
     */
    ...
    public function routeStartup(Controller_Request_Abstract $request)
    {
        foreach ($this->_plugins as $plugin) {
            try {
                ...
                if (Controller_Front::getInstance()->throwExceptions()) {
                    throw $e;
                } else {
    ...
    /**
     * Called before Controller_Front exits its iterations over
     * the route set.
     *
     * @param  Controller_Request_Abstract $request
     * @return void
     ...
     *
     * During the dispatch loop, Controller_Front keeps a
     * Controller_Request_Abstract object, and uses
     * Controller_Dispatcher to dispatch the
     * Controller_Request_Abstract object to controllers/actions.
     *
     * @param  Controller_Request_Abstract $request
     * @return void
     */
