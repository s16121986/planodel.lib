 ...
 */
/**
 * @see Controller_Action_HelperBroker_PriorityStack
 */
require_once 'Library/Controller/Action/HelperBroker/PriorityStack.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Controller_Action_HelperBroker {
	...
	/**
	 * @var Loader_PluginLoader_Interface
	 */
	 ...
	 *
	 * @var Controller_Action_HelperBroker_PriorityStack
	 */
	 ...
	 *
	 * @param  Loader_PluginLoader_Interface $loader
	 * @return void
	 */
	public static function setPluginLoader($loader) {
		if ((null !== $loader) && (!$loader instanceof Loader_PluginLoader_Interface)) {
			require_once 'Library/Controller/Action/Exception.php';
			throw new Controller_Action_Exception('Invalid plugin loader provided to HelperBroker');
		}
	 ...
	 *
	 * @return Loader_PluginLoader
	 */
	public static function getPluginLoader() {
		if (null === self::$_pluginLoader) {
			require_once 'Library/Loader/PluginLoader.php';
			self::$_pluginLoader = new Loader_PluginLoader(array(
						'Controller_Action_Helper' => 'Library/Controller/Action/Helper/',
					));
		}
	 ...
	 *
	 * @param string $path
	 * @param string $prefix Optional; defaults to 'Controller_Action_Helper'
	 * @return void
	 */
	static public function addPath($path, $prefix = 'Controller_Action_Helper') {
		self::getPluginLoader()->addPrefixPath($prefix, $path);
	}
	 ...
	 *
	 * @param Controller_Action_Helper_Abstract $helper
	 * @return void
	 */
	static public function addHelper(Controller_Action_Helper_Abstract $helper) {
	 ...
	 *
	 * @param  string $name
	 * @return Controller_Action_Helper_Abstract
	 */
	...
	public static function getExistingHelper($name) {
		...
		if (!isset($stack->{$name})) {
			require_once 'Library/Controller/Action/Exception.php';
			throw new Controller_Action_Exception('Action helper "' . $name . '" has not been registered with the helper broker');
		}
	 ...
	 *
	 * @return Controller_Action_HelperBroker_PriorityStack
	 */
	public static function getStack() {
		if (self::$_stack == null) {
			self::$_stack = new Controller_Action_HelperBroker_PriorityStack();
		}
	 ...
	 * @return void
	 */
	public function __construct(Controller_Action $actionController) {
	 ...
	 * @param  array $args
	 * @return mixed
	 * @throws Controller_Action_Exception if helper does not have a direct() method
	 */
	public function __call($method, $args) {
		$helper = $this->getHelper($method);
		if (!method_exists($helper, 'direct')) {
			require_once 'Library/Controller/Action/Exception.php';
			throw new Controller_Action_Exception('Helper "' . $method . '" does not support overloading via direct()');
		}
	...
	protected static function _loadHelper($name) {
		...
		} catch (Loader_PluginLoader_Exception $e) {
			require_once 'Library/Controller/Action/Exception.php';
			throw new Controller_Action_Exception('Action Helper by name ' . $name . ' not found', 0, $e);
		}

		$helper = new $class();

		if (!$helper instanceof Controller_Action_Helper_Abstract) {
			require_once 'Library/Controller/Action/Exception.php';
			throw new Controller_Action_Exception('Helper name ' . $name . ' -> class ' . $class . ' is not of type Controller_Action_Helper_Abstract');
		}
