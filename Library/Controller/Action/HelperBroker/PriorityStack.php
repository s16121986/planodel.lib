 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Controller_Action_HelperBroker_PriorityStack implements IteratorAggregate, ArrayAccess, Countable {
	 ...
	 */
	public function push(Controller_Action_Helper_Abstract $helper) {
	...
	public function offsetGet($priorityOrHelperName) {
		if (!$this->offsetExists($priorityOrHelperName)) {
			...
			throw new Controller_Action_Exception('A helper with priority ' . $priorityOrHelperName . ' does not exist.');
		}
	...
	public function offsetSet($priority, $helper) {
		...
		if (!$helper instanceof Controller_Action_Helper_Abstract) {
			require_once 'Library/Controller/Action/Exception.php';
			throw new Controller_Action_Exception('$helper must extend Controller_Action_Helper_Abstract.');
		}
	...
	public function offsetUnset($priorityOrHelperName) {
		if (!$this->offsetExists($priorityOrHelperName)) {
			require_once 'Library/Controller/Action/Exception.php';
			throw new Controller_Action_Exception('A helper with priority or name ' . $priorityOrHelperName . ' does not exist.');
		}
