...
class Captcha {
	...
	private $allowedSymbols = "23456789abcdegikpqsvxyz"; #alphabet without similar symbols (o=0, 1=l, i=j, t=f)
	
	private $keystring = '';
	
	private $_config = array(
		'session_index' => 'captcha_keystring',
		'jpeg_quality' => 90,
		'length' => array(5, 7),
		'width' => 160,
		'height' => 80,
		'no_spaces' => true,
		'fluctuation_amplitude' => 8, # symbol's vertical fluctuation amplitude
		'white_noise_density' => 0.16, //$white_noise_density=0; // no white noise
		'black_noise_density' => 0.03, //$black_noise_density=0; // no black noise
	);
	...
	public function getKeystring() {
		return (isset($_SESSION[$this->session_index]) ? $_SESSION[$this->session_index] : null);
	}

	public function getImage() {
		$fonts = array();
		$fontsdir = dirname(__FILE__) . '/Image/fonts';
		...
		do {
			...
			for ($i = 0; $i < $length; $i++) {
				...
				$y = (($i % 2) * $this->fluctuation_amplitude - $this->fluctuation_amplitude / 2) * $odd + mt_rand(-round($this->fluctuation_amplitude / 3), round($this->fluctuation_amplitude / 3)) + ($this->height - $fontfile_height) / 2;
		...
		$_SESSION[$this->session_index] = $this->keystring;
