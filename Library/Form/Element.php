...
abstract class Element{
	...
	public function getLabel() {
		if (null === $this->_label) {
			$this->_label = new Label(array_merge($this->_options, is_array($this->lableOptions) ? $this->lableOptions : array(), array('text' => $this->label)));
	...
	protected function getJsPlugin($options = array()) {
		if ($this->jsPlugin) {
			...
			return '<script type="text/javascript">Form.initElement("' . $this->id . '","' . (true === $this->jsPlugin ? $this->type : $this->jsPlugin) . '",' . json_encode($options) . ');</script>';
		}
		return '';
	}
