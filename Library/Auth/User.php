...
class User{
	...
	public function login($data) {
		...
		if (!(self::checkData($data, array('login', 'password')) || self::checkData($data, array('provider', 'identifier')))) {
			$this->exception = new Exception(Exception::INCORRECT_DATA);
			return false;
		}
	
		if ($this->enableSocial && isset($data['provider'])) {
			$profile = $this->getProvider($data['provider'])->authenticate($data);
			$authRow = Db::from('user_social_ref', array('id as social_id', 'provider', 'identifier'))
					->joinInner('users', 'users.id=user_social_ref.user_id', array('id', 'status'))
					->where('user_social_ref.provider=?', $data['provider'])
					->where('user_social_ref.identifier=?', $data['identifier'])
					->query()->fetchRow();
		} else {
			$authRow = Db::from('users', '*')
