...
abstract class Provider{
	...
	public static function factory($model) {
		$config = self::getConfig('providers', ucfirst($model));
		if ($config) {
			$cls = 'Auth\Provider\Providers\\' . ucfirst($model);
			$provider = new $cls($config);
			return $provider;
		} else {
			throw new Exception('Provider invalid');
		}
		return null;
	}
	...
	public static function endpoint() {
		$model = Storage::config('provider');
		if ($model && ($provider = self::factory($model))) {
			$provider->endpoint();
		}
		return false;
	}
	...
	public static function login($data) {
		...
		if (isset($data['identifier'])) {
			$authRow = Db::from('user_social_ref', array('id', 'user_id', 'provider', 'identifier'))
				->joinInner('users', 'users.id=user_social_ref.user_id', null)
