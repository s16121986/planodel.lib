...
class Cookies extends Session{
		...
		if ($cookie) {
			...
			if (!empty($user)) {
				...
				if ($this->user->enableSocial && !$flag) {
					$q = Db::from('user_social_ref', array('provider', 'identifier'))
						->where('user_social_ref.user_id=?', $userId)
						->query();
					while ($row = $q->fetch()) {
	...
	public function hash($data) {
		if (isset($data['provider'])) {
			$values = array($data['provider'], $data['identifier']);
		} else {
	...
	public static function setCookies($userId, $hash, $remember = true ) {
		...
		if (!HttpCookies::set(self::COOKIE_AUTH, $cookie, $now->getTimestamp())) {
